...
async function addUser(){
    try{
        ...
        const response = await fetch('https://the-one-api.dev/v2/book', options)
        const data = await response.json()
        console.log(data)
...
async function postUserData() {
    try {
        const options = {
            ...
            body: JSON.stringify({ name: 'james', age:30 })
        }

        const res = await fetch('https://jsonplaceholder.typicode.com/posts', options)  //fake online REST API
